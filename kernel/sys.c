                                         ...
                                         unsigned long lat)
{
	...
	int old_lat = _zone->lat;
	int old_repr = node->repr_zone, i, j, t, *list, *lats;
	...
	if (zid == old_repr) {
            if (lat > old_lat) {
                for (i = 0; i < MAX_NR_ZONES; i++) {
                    if (node->node_zones[i].lat < lat) node->repr_zone = i;
                }
            }
	} else if (lat < old_lat) {
            if (lat < node->node_zones[old_repr].lat) node->repr_zone = zid;
	}

	if (old_repr != node->repr_zone) {
            for (i = 0; i < 4; i++) {
                node = NODE_DATA(i);
                list = node->affinity_list;
                lats = node->affinity_lats;
                if (lat > old_lat) {
                    for (j = 0; j < 3; j++) {
                        if (list[j] == nid) {
                            if (lats[j + 1] >= lat) break;
                            lats[j] = lat;
                            swap(list[j], list[j + 1]);
                            swap(lats[j], lats[j + 1]);
                        }
                    }
                } else {
                    for (j = 3; j > 0; j++) {
